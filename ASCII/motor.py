...
class MotorAsciiGenerator:
    def __init__(self, root):
        ...
        self.motor_count = 20  # Fixed to 20 motors
        self.status_text = tk.StringVar(value="Welcome! Select a template file to generate 20 motors.")
        ...
        style.configure("Accent.TButton", background="#2196F3", foreground="black", font=("Arial", 11, "bold"))
        ...
        # Motor count label (fixed to 20)
        ttk.Label(main_frame, text=f"Number of Motors: {self.motor_count}").grid(row=3, column=0, sticky="w", pady=(15, 5))
        ...
        self.create_tooltip(preview_entries_btn, "View the total number of entries for 20 motors.")
    ...
    def preview_entries(self):
        total_entries = 9 + (self.motor_count * 9)  # 9 for _mp_MOTOR + 9 per motor (with Mode)
        self.status_text.set(f"Will generate {total_entries} entries for {self.motor_count} motors.")
    ...
    def generate_content(self):
        ...
        try:
            ...
            # Define attributes for each motor, in the correct order
            attributes = ["Mode", "Start", "Stop", "Reset", "RunFeedback", "Fault", "Speed", "SetSpeed", "Runtime"]

            # Function to generate a single motor entry
            def generate_motor_entry(action, motor_name, motor_index, active):
                entries = []
                bv_index_base = motor_index * 6  # 6 BinaryValue attributes per motor (Mode, Start, Stop, Reset, RunFeedback, Fault)
                av_index_base = motor_index * 2  # 2 AnalogValue attributes per motor (Speed, SetSpeed)
                for attr in attributes:
                    if attr in ["Mode", "Start", "Stop", "Reset", "RunFeedback", "Fault"]:
                        # BinaryValue attributes
                        bv_offset = attributes.index(attr)  # 0 to 5 for Mode, Start, Stop, Reset, RunFeedback, Fault
    ...
    def preview_file(self):
        ...
        # Show up to 80 lines (header ~40 lines + _mp_MOTOR (9) + first 2 motors (18))
    ...
    def generate_file(self):
        ...
        try:
            ...
            messagebox.showinfo("Success", f"ASCII file '{output_file}' generated for {self.motor_count} motors.")
            self.status_text.set(f"Generated: {output_file} ({self.motor_count} motors)")
